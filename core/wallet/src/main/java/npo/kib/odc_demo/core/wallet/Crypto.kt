...
import npo.kib.odc_demo.core.common_jvm.decodeHex
...
//todo separate keystore to another module ?
object Crypto {
     ...
     */
    fun String.generatePublicKey(): PublicKey {
        val subjectPublicKeyInfo = if (take(30) == "-----BEGIN RSA PUBLIC KEY-----" && takeLast(28) == "-----END RSA PUBLIC KEY-----")
        ...
        if (PKCSObjectIdentifiers.rsaEncryption == subjectPublicKeyInfo.algorithm.algorithm) {
            ...
            val spec = RSAPublicKeySpec(
                BigInteger(modulus.toString()),
                BigInteger(exponent.toString())
            )
     ...
     */
    fun PublicKey.asPlainString(): String {
     ...
     */
    fun PublicKey.asPemString(): String {
        return "-----BEGIN RSA PUBLIC KEY-----\n${asPlainString()}-----END RSA PUBLIC KEY-----"
    }
