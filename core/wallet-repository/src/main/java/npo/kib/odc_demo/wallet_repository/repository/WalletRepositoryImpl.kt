package npo.kib.odc_demo.wallet_repository.repository
...
import npo.kib.odc_demo.datastore.UtilityDataStoreObject.*
import npo.kib.odc_demo.datastore.UtilityDataStoreRepository
import npo.kib.odc_demo.network.api.ServerConnectionStatus
import npo.kib.odc_demo.network.api.ServerConnectionStatus.WALLET_ERROR
import npo.kib.odc_demo.network.api.WalletRequest
...
import npo.kib.odc_demo.wallet.model.*
...
class WalletRepositoryImpl(
    ...
    override suspend fun getLocalUserInfo(): UserInfo = withContext(ioDispatcher) {
        UserInfo(
            defaultDataStoreRepository.readValueOrDefault(USER_NAME),
    ...
    override suspend fun addBanknotesToWallet(banknotes: List<BanknoteWithBlockchain>) =
        withContext(ioDispatcher) {
            banknotes.forEach {
                ...
                it.blocks.forEach { block -> ensureActive(); blockchainRepository.insertBlock(block) }
            }
        }
    ...
    private fun verifySokSign(
        ...
        if (!Crypto.verifySignature(sokHash, sokSignature, bokString.generatePublicKey())) {
