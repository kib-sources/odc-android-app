...
import npo.kib.odc_demo.feature_app.domain.connectivity.P2PConnection
import npo.kib.odc_demo.feature_app.data.connectivity.tcp.P2PConnectionTcpImpl
...
class P2pInstrumentedTest {
    ...
    fun p2pUseCaseTest(): Unit = runBlocking {
        val connectivity: P2PConnection = P2PConnectionTcpImpl(appContext, "192.168.1.117")
        val receiveUseCase = P2PReceiveUseCase(appContext, connectivity)
        receiveUseCase.startDiscovery()

        val res = receiveUseCase.connectionResult.take(2).last()
        myLogs(res)

//        receiveUseCase.connectivity.receivedBytes.collect { myLogs(it.decodeToString()) }
